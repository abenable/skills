#!/usr/bin/env bash
# sms - SMS toolkit — send messages, manage contacts, temp
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/sms"
mkdir -p "$DATA_DIR"

show_help() {
    echo "SMS v$VERSION"
    echo ""
    echo "Usage: sms <command> [options]"
    echo ""
    echo "Commands:"
...
cmd_add() {
    local item="${1:?Usage: sms add <item>}"
